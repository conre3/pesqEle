...
A Tabela \@ref(tab:conre) confronta os CNPJs das empresas que realizaram pesquisas (somente em SP, PR, MT e MS) com a relação de empresas que estão registradas e em dia com o CONRE-3, o CONRE-4 ou o CONRE-6. A Tabela \@ref(fig:conre2) mostra a relação das empresas que não estão registradas ou estão registradas e não estão em dia com o CONRE-3 ou CONRE-6. É importante mencionar que as empresas dessa relação podem ser registradas em conselhos de outras regiões.

```{r conre, message=FALSE}
library(conreCNPJ)
conre3_companies <- conreCNPJ::conre_get_cnpj(3)
conre6_companies <- conreCNPJ::conre_get_cnpj(6)
conre4_companies <- conreCNPJ::conre_get_cnpj(4)
conre_companies <- bind_rows(conre3_companies, 
                             conre4_companies,
                             conre6_companies) %>% 
...
pesq_conre3 <- pesq_details %>% 
  ...
  left_join(select(conre_companies, cnpj, empresa), 'cnpj') 

pesq_conre3 %>%
  distinct(cnpj, .keep_all = TRUE) %>%
  count(Registrado = if_else(!is.na(empresa.y), 'Sim', 'Não')) %>%
  knitr::kable(caption = 'Contagem de empresas registradas no CONRE-3, CONRE-4 ou CONRE-6 que registraram pesquisas no TSE (somente em SP, PR, MT e MS).')
```

```{r conre2, fig.cap='<br/>'}
pesq_conre3 %>% 
  filter(is.na(empresa.y)) %>% 
  count(cnpj, nm) %>% 
  ungroup() %>% 
  arrange(desc(n)) %>% 
  DT::datatable(caption = 'Relação de empresas que registraram pesquisas em SP, MT, MS ou PR e não estão registradas ou não estão em dia com o CONRE-3, CONRE-4 ou CONRE-6.')
```

```{r echo=FALSE, eval=FALSE}
# renderizar para readme
rmarkdown::render('vignettes/pesq.Rmd', 
                  output_format = rmarkdown::md_document(), 
                  output_file = 'README.md', 
                  output_dir = '~/conre3/pesqEle')
```
