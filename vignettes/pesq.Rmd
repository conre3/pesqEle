...
pesq_conre3 %>%
  ...
  knitr::kable(caption = 'Contagem de empresas registradas no CONRE-3 ou CONRE-6 que registraram pesquisas no TSE (somente em SP, PR, MT e MS).')
```
...
pesq_conre3 %>% 
  ...
  ungroup() %>% 
  arrange(desc(n)) %>% 
  DT::datatable(caption = 'Relação de empresas que registraram pesquisas em SP, MT, MS ou PR e não estão registradas ou não estão em dia com o CONRE-3 ou CONRE-6.')
```
